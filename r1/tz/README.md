...
- Деньги не сгорают, никаких подписок, никаких лимитов
...
## ⚠️ Что НЕ реализовывать

- 📁 [`archive/`](archive/) — устаревшие ТЗ под старую подписочную модель (1590/4290/9990 ₽ тарифы, квоты, гибридные списания). Закрытые решения, не возвращаемся.
- 📁 [`../archive_old_ui/`](../archive_old_ui/) — старые HTML-мокапы (cabinet/parser/wallet) под ту же подписочную модель.
- `03_backend_deep_dive.md` — содержит legacy-разделы про квоты/промо (см. баннер в начале файла). Бери оттуда только observability/security/race conditions/testing.

Если что-то актуальное оттуда нужно — оно уже перенесено в текущие файлы (00, 02, 05, 14–18).
...
- ❌ Лимиты с счётчиками «осталось N запросов»
